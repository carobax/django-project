...
from fandango.theater.models import Theater, Showtime, Movie

# Here we are creating a custom management command to load our theater data into
# Django models.
...
# In our case this file is called `load_movies.py` so the command we will use
# to execute this file is `manage.py load_winners final-project/fandango-chapelhill.json`
class Command(BaseCommand):
    help = 'Load theater data into the database'
    ...
    def handle(self, *args, **options):
        ...
        for i, row in enumerate(data):
            ...
            theater_name = row['theaters']
                for theater in theater_name:
                    location_name = code["name"]
                    location_address = code["geo"]
                        for location in location_adress:
